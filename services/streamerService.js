...
// CORS proxy konfigürasyonu (gerekirse)
const CORS_PROXY = ""; // Örn: "https://corsproxy.io/?" veya kendi proxy'niz

// Rate limiting için cache
const cache = new Map();
const CACHE_DURATION = 30000; // 30 saniye

// Aynı anda yapılacak maksimum istek sayısı
...
export const streamerService = {
  ...
  async getVideos(username) {
    ...
    const apiUrl = `https://kick.com/api/v2/channels/${username}/videos`;
  ...
  async getStreamer(username) {
    ...
    try {
      // Paralel istekler yerine sıralı - CORS hatalarını azaltır
      const channelData = await this.fetchWithRetry(apiUrl, { method: "GET" });

      // Video verisi sadece gerekirse al
